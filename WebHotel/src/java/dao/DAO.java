...
public class DAO {
    ...
    public List<Services> getAllServicesByid4() {
        List<Services> list = new ArrayList<>();
        String query = "select s.srid,s.urlimage,s.sname,st.stid,s.price,s.describle from Services as s INNER JOIN ServiceType as st on s.stid = st.stid  where srstatus = 1 and st.stid =4";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            rs = ps.executeQuery();
    ...
    public List<Room> getAllRoom() {
        ...
        String query = "select r.rid,r.urlimage1, rt.roomType,r.peopleNumber,r.rankofRoom,r.bedNumber,r.priceRoom,r.describle from Room as r INNER JOIN RoomType as rt on r.rtid=rt.rtid where rstatus = 1";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            rs = ps.executeQuery();
    ...
    public List<Room> searchRoom(int peopleNumber, String rankofRoom) {
        List<Room> list = new ArrayList<>();
        String query = "select r.rid,r.urlimage1, rt.roomType,r.peopleNumber,r.rankofRoom,r.bedNumber,r.priceRoom,r.describle from Room as r INNER JOIN RoomType as rt on r.rtid=rt.rtid \n"
                + "where rstatus = 1 and r.peopleNumber = ? and r.rankofRoom = ?";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            ps.setInt(1, peopleNumber);
            ps.setString(2, rankofRoom);
            rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new Room(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5), rs.getInt(6), rs.getDouble(7), rs.getString(8)));
            }
    ...
    public List<Room> getRoomByID(String rid) {
        List<Room> list = new ArrayList<>();
        String query = "select r.rid,r.urlimage1, rt.roomType,r.peopleNumber,r.rankofRoom,r.bedNumber,r.priceRoom,r.describle from Room as r INNER JOIN RoomType as rt on r.rtid=rt.rtid where rstatus = 1 and rt.rtid = ?";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            ps.setString(1, rid);
    ...
    public Roomdetail getRoomDetailByRID(String rid) {
        String query = "select r.rid,r.urlimage1,r.urlimage2,r.urlimage3, rt.roomType,r.peopleNumber,r.rankofRoom,r.bedNumber,r.priceRoom,r.describle from Room as r INNER JOIN RoomType as rt on r.rtid=rt.rtid where rstatus = 1 and r.rid = ?";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            ps.setString(1, rid);
            rs = ps.executeQuery();
            while (rs.next()) {
                return new Roomdetail(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getInt(6), rs.getString(7), rs.getInt(8), rs.getDouble(9), rs.getString(10));
            }
        } catch (Exception e) {
        }
        return null;
    ...
    public Room getRoomIForPayByID(String rid) {
        String query = "select r.rid,r.urlimage1, rt.roomType,r.peopleNumber,r.rankofRoom,r.bedNumber,r.priceRoom,r.describle from Room as r INNER JOIN RoomType as rt on r.rtid=rt.rtid where rstatus = 1 and r.rid = ?";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            ps.setString(1, rid);
            rs = ps.executeQuery();
            while (rs.next()) {
                return new Room(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5), rs.getInt(6), rs.getDouble(7), rs.getString(8));
            }
        } catch (Exception e) {
        }
        return null;
    ...
    public QLusers Login(String user, String pass) {
        String query = "select * from QLusers where username = ? and passwork = ?";
        try {
            con = new DBConnect().getConnection();
            ps = con.prepareStatement(query);
            ps.setString(1, user);
